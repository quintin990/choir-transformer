...
const Btn = ({ children, loading, disabled, onClick, style, className = '' }) => (
  <button onClick={onClick} disabled={disabled || loading}
    className={`inline-flex items-center justify-center gap-2 h-10 px-5 rounded-lg text-sm font-semibold transition-all disabled:opacity-30 disabled:cursor-not-allowed ${className}`}
    style={style}>
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mb-7">
        ...
        <p className="text-sm" style={{ color: '#9CB2D6' }}>
          Upload a track. Auralyn will isolate vocals, drums, bass, and more.
        </p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        {/* Left — upload */}
        <Card>
          <CardHeader title="Upload audio" subtitle="MP3 or WAV recommended. Keep under 200 MB." />
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Select value={quality} onValueChange={setQuality}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="hq">High Quality — best separation, slower</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <label className="flex items-start gap-3 cursor-pointer group pt-1">
              <input type="checkbox" checked={rights} onChange={e => setRights(e.target.checked)}
                className="mt-0.5 w-4 h-4 shrink-0 cursor-pointer accent-[#1EA0FF]" />
              <span className="text-xs leading-relaxed transition-colors" style={{ color: rights ? '#EAF2FF' : '#9CB2D6' }}>
            ...
            <Btn
              ...
              disabled={!canSubmit}
              onClick={handleStart}
              style={{ backgroundColor: '#1EA0FF', color: '#fff', width: '100%' }}
            >
              {loading ? stage || 'Working…' : 'Start Separation'}
            </Btn>
          </div>
        </Card>
      </div>
