...
const KIND_COLORS = { stems: 'hsl(var(--color-primary))', reference: 'hsl(var(--color-accent))', match: 'hsl(var(--color-purple))' };
...
function JobRow({ job, onTagChange, editTagsId, setEditTagsId }) {
  ...
  const kindColor = KIND_COLORS[job.kind] || 'hsl(var(--color-muted))';
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {job.mode && (
              <span className="text-[10px] px-1.5 py-0.5 rounded"
                style={{ backgroundColor: 'hsl(var(--color-border))', color: 'hsl(var(--color-muted))' }}>
          ...
          {isActive && (
            <div className="mt-2 space-y-1">
              <div className="flex justify-between text-[11px]" style={{ color: 'hsl(var(--color-muted))' }}>
                <span>{job.stage || 'Processing…'}</span>
                <span className="tabular-nums">{job.progress || 0}%</span>
              </div>
              <Progress value={job.progress || 0} className="h-1" style={{ backgroundColor: 'hsl(var(--color-border))' }} />
            </div>
          )}
          ...
          <div className="mt-2 flex items-center gap-2 flex-wrap">
            {showTagEditor ? (
              <>
                ...
                <button onClick={() => setEditTagsId(null)} className="text-[11px]" style={{ color: 'hsl(var(--color-muted))' }}>Done</button>
              </>
            ) : (
              <>
                {(job.tags || []).length > 0 && (
          ...
          <p className="text-[11px] mt-1.5" style={{ color: 'hsl(var(--color-muted))' }}>
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold" style={{ color: 'hsl(var(--color-text))', letterSpacing: '-0.02em' }}>Jobs</h1>
          <p className="text-sm mt-0.5" style={{ color: 'hsl(var(--color-muted))' }}>{jobs.length} total</p>
        </div>
      ...
      <Card padding="p-4" className="mb-5">
        <div className="flex flex-col sm:flex-row gap-3 mb-3">
          <div className="relative flex-1">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5" style={{ color: 'hsl(var(--color-muted))' }} />
          ...
          <div className="flex gap-1">
            {[
              ...
              <button key={id} onClick={() => setViewMode(id)}
                ...
                style={{
                  backgroundColor: viewMode === id ? 'hsl(var(--color-primary) / 0.09)' : 'hsl(var(--color-border))',
                  color: viewMode === id ? 'hsl(var(--color-primary))' : 'hsl(var(--color-muted))',
                  border: `1px solid ${viewMode === id ? 'hsl(var(--color-primary) / 0.19)' : 'transparent'}`,
                }}>
          ...
          {allTags.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              <button onClick={() => setTagFilter('')}
                ...
                style={{ backgroundColor: !tagFilter ? 'hsl(var(--color-primary))' : 'hsl(var(--color-border))', color: !tagFilter ? 'hsl(var(--color-primary-foreground))' : 'hsl(var(--color-muted))' }}>
                All
              </button>
              {allTags.map(t => (
      ...
      {loading ? (
        <div className="text-center py-16" style={{ color: 'hsl(var(--color-muted))' }}>Loading…</div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-16">
          <List className="w-8 h-8 mx-auto mb-3 opacity-20" style={{ color: 'hsl(var(--color-muted))' }} />
          <p className="text-sm" style={{ color: 'hsl(var(--color-muted))' }}>No jobs match your filters.</p>
        </div>
      ) : viewMode === 'flat' ? (
            ...
            return (
              <div key={pid}>
                <Link to={`${createPageUrl('ProjectDetail')}?id=${pid}`}
                  ...
                  {proj?.artist_name && <span className="text-xs" style={{ color: 'hsl(var(--color-muted))' }}>{proj.artist_name}</span>}
                  <ChevronRight className="w-3.5 h-3.5 opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: 'hsl(var(--color-primary))' }} />
                </Link>
                <div className="pl-4 border-l space-y-2" style={{ borderColor: 'hsl(var(--color-primary) / 0.19)' }}>
                  {pjobs.map(job => (
                    <JobRow key={job.id} job={job} onTagChange={handleTagChange} editTagsId={editTagsId} setEditTagsId={setEditTagsId} />
                  ))}
                </div>
          ...
          {unsorted.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-2.5">
                <List className="w-4 h-4" style={{ color: 'hsl(var(--color-muted))' }} />
                <span className="text-sm font-semibold" style={{ color: 'hsl(var(--color-muted))' }}>Unsorted</span>
                <span className="text-[11px] px-1.5 py-0.5 rounded" style={{ backgroundColor: 'hsl(var(--color-border))', color: 'hsl(var(--color-muted))' }}>
                  {unsorted.length}
                </span>
              </div>
              <div className="pl-4 border-l space-y-2" style={{ borderColor: 'hsl(var(--color-border))' }}>
                {unsorted.map(job => (
                  <JobRow key={job.id} job={job} onTagChange={handleTagChange} editTagsId={editTagsId} setEditTagsId={setEditTagsId} />
                ))}
              </div>
